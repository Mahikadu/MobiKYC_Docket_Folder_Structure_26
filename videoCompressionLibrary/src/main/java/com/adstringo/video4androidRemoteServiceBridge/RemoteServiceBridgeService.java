...
/**
 * http://ajkamath.blogspot.in/2010/05/android-aidl-simplified.html
 * Jni method called from onStartCommand 
 * @author tushar
 *
 */
public class RemoteServiceBridgeService extends Service {

	private Handler serviceHandler;
	//private int status = Prefs.Status.STATUS_FINISHED_OK.ordinal();
	private JniTask jniTask = new JniTask();
	...
	ProgressBackgroundRemoteTask _progressRemote;
	...
	private IFRemoteServiceBridge.Stub myRemoteServiceStub = new IFRemoteServiceBridge.Stub() {

		public void setComplexCommand(String[] command) throws RemoteException {
			_command = command;
		...
		public void setSimpleCommand(String command) throws RemoteException {
			_commandStr = command;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (_cat.equals("Base")) {
			...
			//int notifIcon = android.R.drawable.alert_light_frame;
			int notifIcon = R.drawable.noti_icon;
			...
			_notification = builder.setContentIntent(pendIntent)
					.setAutoCancel(true)
					.setSmallIcon(R.drawable.noti_icon)
					.build();
			//nm.notify(1234567, _notification);
			nm.notify(Prefs.NOTIFICATION_ID, _notification);
			_notification.flags |= Notification.FLAG_FOREGROUND_SERVICE;
			startForeground(Prefs.NOTIFICATION_ID, _notification);

			_progressRemote = new ProgressBackgroundRemoteTask( Prefs.NOTIFICATION_ID, getApplicationContext(), _notification, this);

			serviceHandler = new Handler();
			serviceHandler.post(jniTask);
			return START_STICKY;
		}
	...
	class JniTask implements Runnable {
		...
		public void run() {
			...
			while (true) {
				if (_command != null && _run) {
					...
					try {
						...
						Log.e(Prefs.TAG, "project finished with errors..");
					}
					break;
				}
				else {
			...
			Log.i(Prefs.TAG, "RemoteService: project finished.");
		}
	}
