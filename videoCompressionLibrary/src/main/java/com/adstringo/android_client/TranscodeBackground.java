...
import com.adstringo.DemoClient;
...
public class TranscodeBackground extends AsyncTask<Void, Integer, Integer> {
    ...
    private BaseWizard.IDateCallback callerActivity;
    ...
    public TranscodeBackground(BaseWizard act, IFRemoteServiceBridge remote, int notificationId) {
        _act = act;
        _remote = remote;
    ...
    protected void onPreExecute() {
        callerActivity = (BaseWizard.IDateCallback) _act;
        ...
        if (progressDialogTitle != null)
            ...
            progressDialog.setTitle("Process Status");
        if (progressDialogMessage != null)
            progressDialog.setMessage(progressDialogMessage);
        else
            progressDialog.setMessage("Please wait.\n Click on the home or back to push to the background,\n You will be notified when finished.\n Force stop this transcoding by clicking on Cancel.");
    ...
    protected void onPostExecute(Integer result) {
        ...
        if (_act != null) {
            ...
            callerActivity.compressedFilePath(_act.getOutputFilePath());

            CompressionTechnique comp = new CompressionTechnique(_act.getApplicationContext());
            comp.sendVideoDetails(new File(_act.getInputFilePath()), new File(_act.getOutputFilePath()));
            _act.finish();
            _act.startActivity(new Intent(_act,MainActivity.class));
    ...
    private void showFinishNotification() {
        ...
        if (Prefs.forceStopFlag)
            notifMessage = "AdstringO stopping process.";
        else {
            notifMessage = "AdstringO process finished.";
        }
        ...
        if (Prefs.forceStopFlag) {
            ...
            String notifTitle = "AdstringO process stopped.";
            if (notificationStoppedMessage != null)
                notifTitle = notificationStoppedMessage;


            NotificationCompat.Builder builder = new NotificationCompat.Builder(_act);
            notification = builder.setContentIntent(pIntent)
                    .setSmallIcon(icon)
                    .setAutoCancel(true).setContentTitle(notifTitle)
                    .build();
            ...
            if (_act.getOutputFile() == null) {
                ...
                Intent intent = new Intent(_act, DemoClient.class);
                PendingIntent pIntent = PendingIntent.getActivity(_act.getApplicationContext(), 0, intent, 0);
                String notifTitle = "AdstringO process Finished";
            ...
            PendingIntent pIntent = PendingIntent.getActivity(_act.getApplicationContext(), 0, intent, 0);
            //notification.setLatestEventInfo(_act, "Process failed", "Click for more information.", pIntent);
            NotificationCompat.Builder builder = new NotificationCompat.Builder(_act);
            notification = builder.setContentIntent(pIntent)
                    .setSmallIcon(icon)
                    .setContentText("Click for more information.")
                    .setAutoCancel(true).setContentTitle("AdstringO process failed.")
                    .build();
  ...
  /*  private void showFinishNotification() {
        ...
        if (Prefs.forceStopFlag)
            notifMessage = "Stopping Process";
        else {
            notifMessage = "Process Finished";
        }
        Notification notification = new Notification(icon, notifMessage, when);
        ...
        if (Prefs.forceStopFlag) {
            ...
            String notifTitle = "Process stopped";
            if (notificationStoppedMessage != null)
                notifTitle = notificationStoppedMessage;
            ...
            if (_act.getOutputFile() == null) {
                ...
                Intent intent = new Intent(_act, DemoClient.class);
                PendingIntent pIntent = PendingIntent.getActivity(_act.getApplicationContext(), 0, intent, 0);
                String notifTitle = "Process Finished";
                if (notificationfinishedMessageTitle != null)
                    notifTitle = notificationfinishedMessageTitle;
                notification.setLatestEventInfo(_act, notifTitle, "", pIntent);

                CompressionTechnique comp = new CompressionTechnique(_act.getApplicationContext());
                comp.sendVideoDetails(new File(_act.getInputFilePath()), new File(_act.getOutputFile()));
            } else {
                ...
                //ImageCompresionApplication imageCompresionApplication = new ImageCompresionApplication(_act.getApplicationContext());
                //imageCompresionApplication.
    ...
    private int calcProgress() {
        ...
        if (Prefs.durationOfCurrent != null) {
            ...
            if (Prefs.no4androidLog) { // this means the new transcoding engine
                currentVkLogSize = FileUtils.getVKLogSizeRandomAccess();
                Log.i(Prefs.TAG, "==== getting currentVkLogSize from VK");
            } else {
                currentVkLogSize = FileUtils.get4androidLogSizeRandomAccess();
                Log.i(Prefs.TAG, "==== getting currentVkLogSize from project4Android log");
            }
            ...
            String currentTimeStr = FileUtils.readLastTimeFromLogFileUsingRandomAccess();
